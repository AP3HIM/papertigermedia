...
# solved it. Solving on the very first clue is worth the most.
SCORE_STEPS = [1000, 850, 700, 550, 400, 250]
...
def get_daily_puzzle(for_date):
    ...
    If a puzzle has been explicitly scheduled for that date, use it.
    Otherwise deterministically pick one from the full pool by hashing the
    date — every player who visits on the same day gets the same puzzle,
    and nothing ever renders an empty "no puzzle today" state just because
    nobody scheduled one yet.
    """
    scheduled = DailyPuzzle.objects.filter(date=for_date).first()
    if scheduled is not None:
        return scheduled

    pool = list(DailyPuzzle.objects.order_by("id"))
    if not pool:
        return None
